  ...
  for (const line of lines) {
    try {
      ...
      if (msg.id === 1) {
        ...
        server.stdin.write(JSON.stringify({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"query_knowledge","arguments":{"keyword":"Alight"}}}) + '\n');
      } else if (msg.id === 3) {
        console.log("=== query_knowledge (Alight) ===");
        console.log(msg.result.content[0].text);
        server.stdin.write(JSON.stringify({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"query_knowledge","arguments":{"keyword":"Takemi"}}}) + '\n');
      } else if (msg.id === 4) {
